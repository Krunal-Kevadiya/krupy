...
import krupy
from krupy.types import OptStr, StrOrPath
...
KRUPY_CMD = local.get(
    ...
    str(Path(sys.executable).parent / "krupy.cmd"),
    str(Path(sys.executable).parent / "krupy"),
    # Poetry installs the executable as krupy.cmd in Windows
    "krupy.cmd",
    "krupy",
)

# Executing krupy this way allows to debug subprocesses using debugpy
# See https://github.com/microsoft/debugpy/issues/596#issuecomment-824643237
KRUPY_PATH = (sys.executable, "-m", "krupy")

# Helpers to use with tests designed for old krupy bracket envops defaults
BRACKET_ENVOPS = {
    "autoescape": False,
    "block_end_string": "%]",
    "block_start_string": "[%",
...
def render(tmp_path: Path, **kwargs) -> None:
    ...
    krupy.run_copy(str(PROJECT_TEMPLATE), tmp_path, data=DATA, **kwargs)
