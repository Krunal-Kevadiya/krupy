...
from krupy.errors import InvalidTypeError
from krupy.main import run_copy, run_recopy, run_update

from .helpers import KRUPY_PATH, Spawn, build_file_tree, expect_prompt, render
...
def test_messages_with_inline_text(
    ...
    build_file_tree(
        {
            (src / "krupy.yaml"): (
                ...
                _message_before_copy: Thank you for using our template on {{ _krupy_conf.os }}
                _message_after_copy: Project {{ project_name }} successfully created
                _message_before_update: Updating on {{ _krupy_conf.os }}
    ...
    # copy
    if interactive:
        tui = spawn(KRUPY_PATH + ("copy", "-r", "v1", str(src), str(dst)), timeout=10)
    ...
    # recopy
    if interactive:
        tui = spawn(KRUPY_PATH + ("recopy", "-r", "v1", str(dst)), timeout=10)
        expect_prompt(tui, "project_name", "str")
        tui.sendline("_new")
        tui.expect_exact(pexpect.EOF)
    else:
    ...
    # update
    if interactive:
        tui = spawn(KRUPY_PATH + ("update", str(dst)), timeout=10)
        expect_prompt(tui, "project_name", "str")
    ...
    build_file_tree(
        {
            ...
            (src / "message_before_copy.md.jinja"): (
                """\
                Thank you for using our template on {{ _krupy_conf.os }}
                """
            ),
...
def test_messages_quiet(
    ...
    if interactive:
        tui = spawn(
            KRUPY_PATH + ("copy", "--quiet", "-r", "v1", str(src), str(dst)),
            timeout=10,
        )
        expect_prompt(tui, "project_name", "str")
        tui.sendline("myproj")
    ...
    if interactive:
        tui = spawn(
            KRUPY_PATH + ("recopy", "--quiet", "-r", "v1", str(dst)), timeout=10
        )
        expect_prompt(tui, "project_name", "str")
        tui.sendline("_new")
        tui.expect_exact(pexpect.EOF)
    ...
    # update
    if interactive:
        tui = spawn(KRUPY_PATH + ("update", "--quiet", str(dst)), timeout=10)
        expect_prompt(tui, "project_name", "str")
        tui.sendline("_update")
        tui.expect_exact(pexpect.EOF)
    else:
