...
from krupy.errors import UnsafeTemplateError
from krupy.main import run_copy, run_update
...
def test_copy(
    ...
    build_file_tree({(src / "krupy.yaml"): yaml.safe_dump(spec)})
...
def test_copy_cli(
    ...
    src, dst = map(tmp_path_factory.mktemp, ["src", "dst"])
    build_file_tree(
        {(src / "krupy.yaml"): yaml.safe_dump({"_tasks": ["touch task.txt"]})}
    )
    _, retcode = KrupyApp.run(
        ["krupy", "copy", *(["--UNSAFE"] if unsafe else []), str(src), str(dst)],
        exit=False,
    )
    if unsafe:
        assert retcode == 0
...
def test_update(
        ...
        build_file_tree(
            {
                ...
                "krupy.yaml": yaml.safe_dump(spec_old),
            }
        )
        git("init")
        git("add", ".")
    ...
    with local.cwd(src):
        build_file_tree(
            {
                "krupy.yaml": yaml.safe_dump(spec_new),
            }
        )
        git("add", ".")
...
def test_update_cli(
    ...
    _, retcode = KrupyApp.run(
        ["krupy", "copy", str(src), str(dst)] + unsafe_args,
        exit=False,
    )
    assert retcode == 0
    assert "_commit: v1" in (dst / ".krupy-answers.yml").read_text()
    ...
    with local.cwd(src):
        build_file_tree(
            {
                "krupy.yaml": yaml.safe_dump({"_tasks": ["touch task-new.txt"]}),
            }
        )
        git("add", ".")
        git("commit", "-m2")
