...
from .helpers import KRUPY_CMD, build_file_tree
...
def template_path(tmp_path_factory: pytest.TempPathFactory) -> str:
    ...
    build_file_tree(
        {
            (root / "{{ _krupy_conf.answers_file }}.jinja"): (
                """\
...
def test_cli_data_parsed_by_question_type(
    ...
    run_result = KrupyApp.run(
        ["krupy", "copy", f"--data=question={answer}", str(src), str(dst), "--quiet"],
        exit=False,
    )
    assert run_result[1] == 0
...
def test_data_cli_takes_precedence_over_data_file(
    ...
    assert run_result[1] == 0
    actual_answers = yaml.safe_load((dst / ".krupy-answers.yml").read_text())
    expected_answers = {
        "_src_path": str(src),
...
def test_good_cli_run_dot_config(
    ...
    Added based on the discussion: https://github.com/Krunal-Kevadiya/krupy/discussions/859
    """
...
def test_help() -> None:
    _help = KRUPY_CMD("--help-all")
    assert "krupy copy [SWITCHES] template_src destination_path" in _help
    assert "krupy update [SWITCHES] [destination_path=.]" in _help


def test_copy_help() -> None:
    _help = KRUPY_CMD("copy", "--help")
    assert "krupy copy [SWITCHES] template_src destination_path" in _help


def test_update_help() -> None:
    _help = KRUPY_CMD("update", "--help")
...
def test_python_run() -> None:
    cmd = [sys.executable, "-m", "krupy", "--help-all"]
