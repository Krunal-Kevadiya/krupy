    ...
    build_file_tree(
        {
            ...
            (root / "api_project" / "[[ _krupy_conf.answers_file ]].tmpl"): (
            ...
            (root / "conf_project" / "[[ _krupy_conf.answers_file ]].tmpl"): (
                "[[ _krupy_answers|to_nice_yaml ]]"
            ),
            (root / "krupy.yml"): (
                f"""\
...
def test_copy_subdirectory_config(template_path: str, tmp_path: Path) -> None:
    krupy.run_copy(template_path, tmp_path, defaults=True, overwrite=True)
...
def test_update_subdirectory(template_path: str, tmp_path: Path) -> None:
    ...
    krupy.run_update(dst_path=tmp_path, defaults=True, overwrite=True)
...
def test_update_subdirectory_from_root_path(
        ...
        build_file_tree(
            {
                ...
                "{{ _krupy_conf.answers_file }}.jinja": "{{ _krupy_answers|to_nice_yaml }}",
            }
        )
        git("init")
        git("add", ".")
    ...
    krupy.run_copy(
        ...
        defaults=True,
        overwrite=True,
        answers_file=".custom.krupy-answers.yaml",
    )
    ...
    answers = yaml.safe_load(
        (dst / "subfolder" / ".custom.krupy-answers.yaml").read_bytes()
    )
...
def test_new_version_uses_subdirectory(
    ...
    # in v2 it moves all things into a subdir and adds that key to krupy.yml.
    ...
    build_file_tree(
        {
            ...
            (src / "{{_krupy_conf.answers_file}}.jinja"): (
                "{{_krupy_answers|to_nice_yaml}}"
            ),
        }
    )
    ...
    krupy.run_copy(str(src), dst, defaults=True, overwrite=True)
    assert (dst / "README.md").exists()
    assert "_commit: v1" in (dst / ".krupy-answers.yml").read_text()
    ...
    with local.cwd(src):
        ...
        Path("{{_krupy_conf.answers_file}}.jinja").rename(
            subdir / "{{_krupy_conf.answers_file}}.jinja"
        )

        # Add the subdirectory option to krupy.yml
        Path("krupy.yml").write_text(f"_subdirectory: {subdir}")
    ...
    krupy.run_update(dst_path=dst, defaults=True, overwrite=True, conflict=conflict)
    assert "_commit: v2" in (dst / ".krupy-answers.yml").read_text()
...
def test_new_version_changes_subdirectory(
    ...
    build_file_tree(
        {
            (src / "krupy.yml"): "_subdirectory: subdir1\n",
            (src / "subdir1" / "[[_krupy_conf.answers_file]].tmpl"): (
                "[[_krupy_answers|to_nice_yaml]]\n"
            ),
    ...
    with local.cwd(src):
        ...
        # Update krupy.yml to reflect this change
        Path("krupy.yml").write_text("_subdirectory: subdir2\n")
