...
from krupy import Worker
from krupy.errors import InvalidTypeError
from krupy.types import AnyByStrDict, OptStr
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "krupydefault",
            [Prompt("templated_default", "str"), "krupydefault"],
        ),
        (
            {
                "templated_type": {
                    "type": "[% if main == 'krupy' %]int[% endif %]",
                    "default": "0",
                },
            },
            0,
        ...
        (
            ...
            main_default,
            [
                Prompt("templated_help", "str", "THIS krupy HELP IS TEMPLATED"),
                "krupy",
            ],
        ),
        (
...
def test_templated_prompt(
    ...
    build_file_tree(
        {
            (src / "krupy.yml"): json.dumps(questions_combined),
...
def test_templated_prompt_invalid(
    ...
    build_file_tree(
        {
            src / "krupy.yml": yaml.safe_dump(questions),
...
def test_templated_prompt_with_conditional_choices(
    ...
    tui = spawn(
        KRUPY_PATH + ("copy", f"--data=cloud={cloud}", str(src), str(dst)),
        timeout=10,
    )
