...
from krupy import run_copy, run_update
from krupy.types import OptStr
...
def test_api(template_path: str, tmp_path: Path) -> None:
    """Test krupy correctly processes advanced questions and answers through API."""
    run_copy(
        template_path,
        tmp_path,
        {
...
def test_cli_interactive(template_path: str, tmp_path: Path, spawn: Spawn) -> None:
    """Test krupy correctly processes advanced questions and answers through CLI."""
    tui = spawn(KRUPY_PATH + ("copy", template_path, str(tmp_path)), timeout=10)
...
def test_api_str_data(template_path: str, tmp_path: Path) -> None:
    """Test krupy when all data comes as a string.
...
def test_cli_interatively_with_flag_data_and_type_casts(
    ...
    """Assert how choices work when krupy is invoked with --data interactively."""
    tui = spawn(
        KRUPY_PATH
        + (
            "copy",
...
def test_tui_inherited_default(
    ...
    build_file_tree(
        {
            ...
            (src / "answers.json.jinja"): "{{ _krupy_answers|to_nice_json }}",
    ...
    with local.cwd(src):
        ...
        git("tag", "1")
    tui = spawn(KRUPY_PATH + ("copy", str(src), str(dst)), timeout=10)
    ...
    build_file_tree(
        {
            ...
            (src / "answers.json.jinja"): "{{ _krupy_answers|to_json }}",
        }
    )
    tui = spawn(KRUPY_PATH + ("copy", str(src), str(dst)), timeout=10)
    expect_prompt(
...
def test_multi_template_answers(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    build_file_tree(
        {
            (tpl1 / "krupy.yml"): "q1: a1",
            (tpl1 / "{{ _krupy_conf.answers_file }}.jinja"): (
                "{{ _krupy_answers|to_nice_yaml }}"
            ),
            (tpl2 / "krupy.yml"): "{_answers_file: two.yml, q2: a2}",
            (tpl2 / "{{ _krupy_conf.answers_file }}.jinja"): (
                "{{ _krupy_answers|to_nice_yaml }}"
            ),
        }
    )
    ...
    with local.cwd(dst):
        ...
        git("commit", "-m2")
        # Check contents
        answers1 = yaml.safe_load(Path(".krupy-answers.yml").read_text())
