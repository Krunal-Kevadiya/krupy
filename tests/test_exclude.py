...
from krupy.main import run_copy
from krupy.template import DEFAULT_EXCLUDE
...
def test_config_exclude(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    build_file_tree({src / "krupy.yml": "_exclude: ['*.txt']", src / "aaaa.txt": ""})
...
def test_config_exclude_extended(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    assert not (dst / "aaaa.txt").exists()
    assert not (dst / "krupy.yml").exists()


def test_config_include(tmp_path_factory: pytest.TempPathFactory) -> None:
    src, dst = map(tmp_path_factory.mktemp, ("src", "dst"))
    build_file_tree(
        {src / "krupy.yml": "_exclude: ['!.svn']", src / ".svn" / "hello": ""}
    )
    run_copy(str(src), dst, quiet=True)
...
def test_config_exclude_with_subdirectory(
    ...
    assert "krupy.yml" in DEFAULT_EXCLUDE

    src, dst = map(tmp_path_factory.mktemp, ("src", "dst"))
    build_file_tree(
        {
            src / "krupy.yml": "_subdirectory: 'template'",
            src / "template" / "krupy.yml": "",
        }
    )
    run_copy(str(src), dst, quiet=True)
    assert (dst / "krupy.yml").exists()
    ...
    src, dst = map(tmp_path_factory.mktemp, ("src", "dst"))
    build_file_tree(
        {src / "krupy.yml": "_subdirectory: '.'", src / "template" / "krupy.yml": ""}
    )
    run_copy(str(src), dst, quiet=True)
    assert not (dst / "template" / "krupy.yml").exists()
    ...
    src, dst = map(tmp_path_factory.mktemp, ("src", "dst"))
    build_file_tree({src / "krupy.yml": "", src / "template" / "krupy.yml": ""})
    run_copy(str(src), dst, quiet=True)
    assert not (dst / "template" / "krupy.yml").exists()
