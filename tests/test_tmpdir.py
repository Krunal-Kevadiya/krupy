...
def template_path(tmp_path_factory: pytest.TempPathFactory) -> str:
    ...
    build_file_tree(
        {
            root / "krupy.yml": "favorite_app: Krupy",
...
def test_api(
    ...
    assert (dst / "fav.txt").read_text() == "Krupy"
...
def test_cli(
    ...
    _git("commit", "-m", "Initial commit")
    # Recopy
    run_result = KrupyApp.run(["krupy", "recopy", "-fqrv1", str(dst)], exit=False)
    assert run_result[1] == 0
    assert (dst / "fav.txt").read_text() == "Krupy"
    assert not (dst / "v2").exists()
    empty_dir(tmp)
    # Update
    run_result = KrupyApp.run(["krupy", "update", "-fq", str(dst)], exit=False)
    assert run_result[1] == 0
    assert (dst / "fav.txt").read_text() == "Krupy"
    assert (dst / "v2").read_text() == "true"
    empty_dir(tmp)
