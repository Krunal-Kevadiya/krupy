...
from krupy import Worker, run_copy, run_update
from krupy.errors import ShallowCloneWarning
from krupy.vcs import checkout_latest_tag, clone, get_git_version, get_repo
...
def test_get_repo() -> None:
    ...
    assert (
        get("https://github.com/Krunal-Kevadiya/krupy.git")
        == "https://github.com/Krunal-Kevadiya/krupy.git"
    )

    assert (
        get("https://github.com/Krunal-Kevadiya/krupy")
        == "https://github.com/Krunal-Kevadiya/krupy.git"
    )
    assert (
    ...
    assert (
        get("gh:/Krunal-Kevadiya/krupy.git") == "https://github.com/Krunal-Kevadiya/krupy.git"
    )
    assert get("gh:Krunal-Kevadiya/krupy.git") == "https://github.com/Krunal-Kevadiya/krupy.git"
    assert get("gl:Krunal-Kevadiya/krupy.git") == "https://gitlab.com/Krunal-Kevadiya/krupy.git"
    assert get("gh:Krunal-Kevadiya/krupy") == "https://github.com/Krunal-Kevadiya/krupy.git"
    assert get("gl:Krunal-Kevadiya/krupy") == "https://gitlab.com/Krunal-Kevadiya/krupy.git"
...
@pytest.mark.impure
def test_clone() -> None:
    tmp = clone("https://github.com/Krunal-Kevadiya/krupy.git")
    assert tmp
...
def test_shallow_clone(tmp_path: Path, recwarn: pytest.WarningsRecorder) -> None:
    ...
    # checked in `clone()`.
    src_path = str(tmp_path / "krupytest")
    git("clone", "--depth=2", "https://github.com/Krunal-Kevadiya/krupy.git", src_path)
...
def test_removes_temporary_clone(tmp_path: Path) -> None:
    src_path = "https://github.com/Krunal-Kevadiya/krupytest.git"
    with Worker(
...
def test_dont_remove_local_clone(tmp_path: Path) -> None:
    src_path = str(tmp_path / "krupytest")
    git("clone", "https://github.com/Krunal-Kevadiya/krupytest.git", src_path)
    with Worker(
        src_path=src_path, dst_path=tmp_path, defaults=True, unsafe=True
    ) as worker:
        worker.run_copy()
...
def test_update_using_local_source_path_with_tilde(tmp_path: Path) -> None:
    ...
    worker = run_update(
        ...
        defaults=True,
        overwrite=True,
        answers_file=".krupy-answers.krupytest.yml",
        unsafe=True,
    )
    assert worker.answers.combined["_src_path"] == user_src_path
...
def test_select_latest_version_tag(
    ...
    with local.cwd(src):
        git("init")
        Path("{{ _krupy_conf.answers_file }}.jinja").write_text(
    ...
    answers = yaml.safe_load((dst / ".krupy-answers.yml").read_text())
