...
from krupy.types import AnyByStrDict
...
def test_project_not_found(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        krupy.run_copy("foobar", tmp_path)

    with pytest.raises(ValueError):
        krupy.run_copy(__file__, tmp_path)
...
def test_copy(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "README.txt").read_text() == "This is the README for Krupy.\n"
...
def test_exclude_extends(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    krupy.run_copy(str(src), dst, exclude=["*.txt"])
...
def test_exclude_replaces(tmp_path_factory: pytest.TempPathFactory) -> None:
    """Exclude in krupy.yml replaces default values."""
    src, dst = map(tmp_path_factory.mktemp, ("src", "dst"))
    build_file_tree(
        {
            ...
            src / "krupy.yaml.jinja": "purpose: template inception",
            src / "krupy.yml": "_exclude: ['*.json']",
        }
    )
    krupy.run_copy(str(src), dst, exclude=["*.txt"])
    ...
    assert (dst / "krupy.yml").exists()
    assert (dst / "krupy.yaml").is_file()


def test_skip_if_exists(tmp_path: Path) -> None:
    krupy.run_copy(str(Path("tests", "demo_skip_dst")), tmp_path)
    krupy.run_copy(
        "tests/demo_skip_src",
        tmp_path,
...
def test_skip_if_exists_rendered_patterns(tmp_path: Path) -> None:
    krupy.run_copy("tests/demo_skip_dst", tmp_path)
    krupy.run_copy(
        "tests/demo_skip_src",
        tmp_path,
        data={"name": "meh"},
...
def test_empty_dir(tmp_path_factory: pytest.TempPathFactory, generate: bool) -> None:
    ...
    krupy.run_copy(str(src), dst, {"do_it": generate}, defaults=True, overwrite=True)
...
def test_commit_hash(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    build_file_tree(
        {
            src / "commit.jinja": "{{_krupy_conf.vcs_ref_hash}}",
            src / "tag.jinja": "{{_krupy_answers._commit}}",
        }
    )
    with local.cwd(src):
        git("init")
...
def test_value_with_forward_slash(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    build_file_tree(
        {
            (src / "{{ filename.replace('.', _krupy_conf.sep) }}.txt"): (
                "This is template."
            ),
        }
    )
    krupy.run_copy(str(src), dst, data={"filename": "a.b.c"})
...
def test_validate_init_data(
    ...
    with expected:
        krupy.run_copy(str(src), dst, data={"q": value})
...
def test_required_question_without_data(
    ...
    with pytest.raises(ValueError, match='Question "question" is required'):
        krupy.run_copy(str(src), dst, defaults=True)
