...
import krupy
from krupy.errors import DirtyLocalWarning
...
def test_copy(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    with pytest.warns(DirtyLocalWarning):
        krupy.run_copy(str(src), dst, data=DATA, vcs_ref="HEAD", quiet=True)
...
def test_copy_dirty_head(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    with local.cwd(src):
        ...
        git("add", "tracked")
        git("commit", "-m1")
    krupy.run_copy(str(src), dst, vcs_ref="HEAD")
...
def test_update(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    # HACK https://github.com/Krunal-Kevadiya/krupy/issues/461
