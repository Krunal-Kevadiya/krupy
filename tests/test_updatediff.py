...
from krupy.cli import KrupyApp
from krupy.errors import UserMessageError
from krupy.main import Worker, run_copy, run_update
...
def test_updatediff(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    answers = target / ".krupy-answers.yml"
    commit = git["commit", "--all"]
    # Run krupy 1st time, with specific tag
    KrupyApp.run(
        [
            "krupy",
            "copy",
    ...
    with local.cwd(target):
        ...
        KrupyApp.run(["krupy", "update", "--defaults", "--UNSAFE"], exit=False)
        assert answers.read_text() == dedent(
            f"""\
            # Changes here will be overwritten by Krupy
            _commit: v0.0.2
        ...
        KrupyApp.run(
            ["krupy", "update", "--defaults", "--vcs-ref=HEAD"],
            exit=False,
        )
...
def test_update_from_tagged_to_head(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    example = dst / "example"
    answers_file = dst / ".krupy-answers.yml"
...
def test_skip_update(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    with local.cwd(src):
        build_file_tree(
            {
                "krupy.yaml": "_skip_if_exists: [skip_me]",
                "{{ _krupy_conf.answers_file }}.jinja": "{{ _krupy_answers|to_yaml }}",
                "skip_me": "1",
            }
        )
        git("init")
...
@pytest.mark.parametrize(
    "answers_file", [None, ".krupy-answers.yml", ".custom.krupy-answers.yaml"]
)
def test_overwrite_answers_file_always(
    ...
    with local.cwd(src):
        ...
        git("commit", "-m1")
        git("tag", "1")
        build_file_tree({"krupy.yaml": "question_1: false"})
        git("commit", "-am2")
        git("tag", "2")
    ...
    with local.cwd(dst):
        ...
        git("add", ".")
        git("commit", "-m1")
        # When updating, the only thing to overwrite is the krupy answers file,
    ...
    answers = yaml.safe_load(
        (dst / (answers_file or ".krupy-answers.yml")).read_bytes()
    )
...
def test_file_removed(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    with local.cwd(dst):
        ...
        git("commit", "-m2")
    # All files exist
    assert (dst / ".krupy-answers.yml").is_file()
...
def test_update_in_repo_subdirectory(
    ...
    assert (dst / subdir / ".krupy-answers.yml").is_file()
...
def test_update_needs_more_context(
    ...
    with local.cwd(dst):
        if api:
            ...
            KrupyApp.run(["krupy", "copy", str(src), "."], exit=False)
        git("init")
        git("add", ".")
        git("commit", "-m1")
        build_file_tree(
