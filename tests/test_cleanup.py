...
def test_cleanup(tmp_path: Path) -> None:
    """Krupy creates dst_path, fails to copy and removes it."""
    dst = tmp_path / "new_folder"
    with pytest.raises(CalledProcessError):
        krupy.run_copy("./tests/demo_cleanup", dst, quiet=True, unsafe=True)
    assert not dst.exists()


def test_do_not_cleanup(tmp_path: Path) -> None:
    """Krupy creates dst_path, fails to copy and keeps it."""
    dst = tmp_path / "new_folder"
    with pytest.raises(CalledProcessError):
        krupy.run_copy(
...
def test_no_cleanup_when_folder_existed(tmp_path: Path) -> None:
    """Krupy will not delete a folder if it didn't create it."""
...
def test_no_cleanup_when_template_in_parent_folder(tmp_path: Path) -> None:
    """Krupy will not delete a local template in a parent folder."""
    src = tmp_path / "src"
    src.mkdir()
    dst = tmp_path / "dst"
    dst.mkdir()
    ...
    cwd.mkdir()
    with local.cwd(cwd):
        krupy.run_copy(str(Path("..", "src")), dst, quiet=True)
    assert src.exists()
