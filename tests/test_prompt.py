...
from krupy.types import StrOrPath
...
MARIO_TREE: Mapping[StrOrPath, Union[str, bytes]] = {
    ...
    "[[ _krupy_conf.answers_file ]].tmpl": "[[_krupy_answers|to_nice_yaml]]",
}
...
def test_copy_default_advertised(
    ...
    with local.cwd(dst):
        ...
        tui = spawn(
            KRUPY_PATH + ("copy", str(src), ".", "--vcs-ref=v1") + args, timeout=10
        )
        ...
        assert "_commit: v1" in Path(".krupy-answers.yml").read_text()
        # Update subproject
        git_save()
        assert "_commit: v1" in Path(".krupy-answers.yml").read_text()
        tui = spawn(KRUPY_PATH + ("update",), timeout=30)
        # Check what was captured
        expect_prompt(tui, "in_love", "bool")
        tui.expect_exact("(Y/n)")
        tui.sendline()
        ...
        tui.sendline()
        tui.expect_exact(pexpect.EOF)
        assert "_commit: v2" in Path(".krupy-answers.yml").read_text()
...
def test_when(
    ...
    build_file_tree(
        {
            (src / "krupy.yml"): yaml.dump(questions),
            (src / "[[ _krupy_conf.answers_file ]].tmpl"): (
...
def test_partial_interrupt(
    ...
    # interrupt.
    # We won't want our custom exception (KrupyAnswersInterrupt) propagating
...
def test_var_name_value_allowed(
    ...
    See https://github.com/Krunal-Kevadiya/krupy/pull/780 for more details.
    """
    src, dst = map(tmp_path_factory.mktemp, ("src", "dst"))
...
def test_required_text_question(
    ...
    if isinstance(expected_answer, ValueError):
        ...
        assert not (dst / ".krupy-answers.yml").exists()
    else:
        tui.expect_exact(pexpect.EOF)
        answers = yaml.safe_load((dst / ".krupy-answers.yml").read_text())
        assert answers == {
