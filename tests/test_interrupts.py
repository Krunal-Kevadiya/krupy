...
from krupy.errors import KrupyAnswersInterrupt
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # it as a KrupyAnswersInterrupt.
        KrupyAnswersInterrupt(Mock(), Mock(), Mock()),
        KeyboardInterrupt,
    ],
)
def test_keyboard_interrupt(
    ...
    with patch("krupy.main.unsafe_prompt", side_effect=side_effect):
...
def test_multiple_questions_interrupt(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    with patch(
        ...
        with pytest.raises(KrupyAnswersInterrupt) as err:
            worker.run_copy()
