...
It is important that you understand how Krupy works. It has 2 kinds of configurations:

1. **Settings** for Krupy itself. This includes things as minimal Krupy version
...
Krupy reads **settings** from these sources, in this order of priority:

1. Command line or API arguments.
1. [The krupy.yml file][the-krupyyml-file]. Settings here always start with an
   underscore (e.g. `_min_krupy_version`).
...
Krupy obtains **answers** from these sources, in this order of priority:

1. Command line or API arguments.
1. Asking the user. Notice that Krupy will not ask any questions answered in the
   previous source.
1. [Answer from last execution][the-krupy-answersyml-file].
1. Default values defined in [the krupy.yml file][the-krupyyml-file].

## The krupy.yml file

The `krupy.yml` (or `krupy.yaml`) file is found in the root of the template, and it is
...
For each key found, Krupy will prompt the user to fill or confirm the values before
...
Apart from the simplified format, as seen above, Krupy supports a more advanced format
...
-   **secret**: When `true`, it hides the prompt displaying asterisks (`*****`) and
    doesn't save the answer in [the answers file][the-krupy-answersyml-file]. When
...
**valid YAML** and respect **Krupy's structure**. That is why we explicitly wrap some
...
The `krupy.yml` file supports multiple documents as well as using the `!include` tag to
include settings and questions from other YAML files. This allows you to split up a
larger `krupy.yml` and enables you to reuse common partial sections from your
...
!!! example
    ...
    This would be a valid `krupy.yml` file:
...
Krupy will generate this structure:
...
You can either use any separator, like `.`, and replace it with `_krupy_conf.sep`, like
...
`krupy.yml` file][the-krupyyml-file]**.

### `answers_file`

-   Format: `str`
-   CLI flags: `-a`, `--answers-file`
-   Default value: `.krupy-answers.yml`
...
Don't forget to read [the docs about the answers file][the-krupy-answersyml-file].
...
-   CLI flags: `-C`, `--no-cleanup` (used to disable this setting; only available in
    `krupy copy` subcommand)
-   Default value: `True`

When Krupy creates the destination path, if there's any failure when rendering the
template (either in the rendering process or when running the [tasks][tasks]), Krupy
will delete that folder.

Krupy will never delete the folder if it didn't create it. For this reason, when
running `krupy update`, this setting has no effect.

!!! info

    Not supported in `krupy.yml`.

### `conflict`

-   Format: `Literal["rej", "inline"]`
-   CLI flags: `-o`, `--conflict` (only available in `krupy update` subcommand)
-   Default value: `inline`

When updating a project, sometimes Krupy doesn't know what to do with a diff code hunk.
...
-   Format: `Int`
-   CLI flags: `-c`, `--context-lines` (only available in `krupy update` subcommand)
-   Default value: `1`

During a project update, Krupy needs to compare the template evolution with the
...
The more lines you use, the more accurate Krupy will be when detecting conflicts. But
you will also have more conflicts to solve by yourself. FWIW, Git uses 3 lines by
default.

The less lines you use, the less conflicts you will have. However, Krupy will not be so
...
This cannot be defined in `krupy.yml`, where its equivalent would be just normal
...
!!! example
    ...
    ```shell
    krupy copy -fd 'user_name=Manuel Calavera' template destination
    ```
...
!!! info
    ...
    Not supported in `krupy.yml` or API calls. Only supported through the CLI.
...
!!! example
    ...
    ```shell
    krupy copy --data-file input.yml template destination
    ```

    is equivalent to passing its content as key-value pairs:

    ```shell
    krupy copy -d 'user_name=Manuel Calavera' -d 'age=7' -d 'height=1.83' template destination
    ```
    ...
    ```shell
    krupy copy -d 'user_name=Bilbo Baggins' --data-file input.yml template destination
    ```
...
Configurations for the Jinja environment. Krupy uses the Jinja defaults whenever
...
!!! warning
    ...
    Krupy and older had different, bracket-based defaults.

    If your template was created for Krupy, you need to add this configuration to
    your `krupy.yaml` to keep it working just like before:
    ...
    By specifying this, your template will be compatible with Krupy.

    Krupy will apply these older defaults if your [min_krupy_version][] is lower
    than 0.

    Krupy no longer uses the old defaults independent of [min_krupy_version][].
...
-   CLI flags: `-x`, `--exclude`
-   Default value:
    `["krupy.yaml", "krupy.yml", "~*", "*.py[co]", "__pycache__", ".git", ".DS_Store", ".svn"]`
...
!!! info
    ...
    When you define this parameter in `krupy.yml`, it will **replace** the default
    value.

    In this example, for instance, `"krupy.yml"` will **not** be excluded:
...
!!! info
    ...
    defined in `krupy.yml` (or the defaults, if missing).

    Instead, CLI/API definitions **will extend** those from `krupy.yml`.


    !!! example "Example CLI usage to copy only a single file from the template"

        ```shell
        krupy copy --exclude '*' --exclude '!file-i-want' ./template ./destination
        ```

### `force`

-   Format: `bool`
-   CLI flags: `-f`, `--force` (N/A in `krupy update`)
-   Default value: `False`
...
-   Format: `bool`
-   CLI flags: `--overwrite` (N/A in `krupy update` because it's implicit)
-   Default value: `False`
...
You don't need to tell your template users to install these extensions: Krupy depends
on them, so they are always installed when Krupy is installed.

!!! warning

    Including an extension allows Krupy to execute uncontrolled code, thus making the
    template potentially more dangerous. Be careful about what extensions you install.

!!! info "Note to template writers"

    You must inform your users that they need to install the extensions alongside Krupy,
    ...
    ```shell
    # with pip, in the same virtualenv where Krupy is installed
...
!!! hint
    ...
    -   From [cookiecutter](https://cookiecutter.readthedocs.io/en/1.7.2/):
        ...
        -   [cookiecutter.extensions.JsonifyExtension](https://cookiecutter.readthedocs.io/en/latest/advanced/template_extensions.html#jsonify-extension):
            provides a `jsonify` filter, to format a dictionary as JSON. Note that Krupy
        ...
        -   [cookiecutter.extensions.RandomStringExtension](https://cookiecutter.readthedocs.io/en/latest/advanced/template_extensions.html#random-string-extension):
            ...
            Note that Krupy natively provides the `ans_random` and `hash` filters that can
    ...
    -   [krupy_templates_extensions.TemplateExtensionLoader](https://github.com/Krunal-Kevadiya/krupy-templates-extensions):
    ...
    -   [jinja2_jsonschema.JsonSchemaExtension](https://github.com/Krunal-Kevadiya/jinja2-jsonschema):
...
The message is suppressed when Krupy is run in [quiet mode][quiet].
...
Specifies the minimum required version of Krupy to generate a project from this
...
!!! info
    ...
    If Krupy detects that there is a major version difference, it will warn you about
...
!!! question "Can I have multiple templates in a single repo using this option?"
    ...
    The Krupy recommendation is: **1 template = 1 Git repository**.

    Why? Unlike almost all other templating engines, Krupy supports
    [smart project updates](updating.md). For that, Krupy needs to know in which version it
    ...
    So, in Krupy, the subdirectory option is just there to let template owners separate
    ...
    !!! example
        ...
        1.  See [the answers file docs][the-krupy-answersyml-file] to understand.
...
An empty suffix is also valid, and will instruct Krupy to copy and render _every file_,
...
templates suffix is _not_ empty, Krupy will abort and print an error message.
...
!!! warning
    ...
    Krupy and older had a different default value: `.tmpl`. If you wish to keep it,
    add it to your `krupy.yml` to keep it future-proof.

    Krupy will apply that old default if your [min_krupy_version][] is lower
    than 0.

    Krupy no longer uses the old default independent of [min_krupy_version][].
...
Krupy templates can use dangerous features that allow arbitrary code execution:

-   [Jinja extensions][jinja_extensions]
-   [Migrations][migrations]
-   [Tasks][tasks]

Therefore, these features are disabled by default and Krupy will raise an error (and
...
`v2.0.0a1`. Krupy will copy by default `v1.0.0` unless you add `--prereleases`.

<!-- prettier-ignore-start -->
Also, if you run [`krupy update`][krupy.cli.KrupyUpdateSubApp], Krupy would ignore
the `v2.0.0a1` tag unless this flag is enabled.
<!-- prettier-ignore-end -->

!!! warning

    This behavior is new from Krupy. Before that release, prereleases were
    never ignored.
...
-   Format: `dict`
-   CLI flags: N/A
-   Default value: `{"is_visible_count": false, "is_visible_mark": false, "is_visible_type": false, "is_visible_default_value": false}`

Configurations for the Krupy environment.
...
Krupy supports matching names against patterns in a gitignore style fashion. This works
...
For example, with the following settings in your `krupy.yml` file would exclude all
...
If the destination path exists and a `.krupy-answers.yml` file is present there, it
will be used to load the last user's answers to the questions made in [the `krupy.yml`
file][the-krupyyml-file].

This makes projects easier to update because when the user is asked, the default answers
will be the last ones they used.

The file **must be called exactly `{{ _krupy_conf.answers_file }}.jinja`** (or ended
...
The default name will be `.krupy-answers.yml`, but [you can define a different default
path for this file][answers_file].

The file must have this content:

```yaml+jinja
# Changes here will be overwritten by Krupy; NEVER EDIT MANUALLY
...
The builtin `_krupy_answers` variable includes all data needed to smooth future updates
of this project. This includes (but is not limited to) all JSON-serializable values
declared as user questions in [the krupy.yml file][the-krupyyml-file].

As you can see, you also have the power to customize what will be logged here. Keys that
start with an underscore (`_`) are specific to Krupy. Other keys should match questions
in `krupy.yml`.
...
Well, don't worry. Krupy has you covered. You just need to use a different answers file
for each one. All of them contain a `{{ _krupy_conf.answers_file }}.jinja` file [as
specified above][the-krupy-answersyml-file]. Then you apply all the templates to the
same project:
...
```shell
krupy update -a .krupy-answers.main.yml
