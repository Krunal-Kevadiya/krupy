...
Krupy enables the code lifecycle management for generated projects. There are several
...
1. The destination folder includes [a valid `.krupy-answers.yml`
   file][the-krupy-answersyml-file].
...
When updating, Krupy will do its best to respect your project evolution by using the
answers you provided when copied last time. However, sometimes it's impossible for
Krupy to know what to do with a diff code hunk. In those cases, krupy handles the
...
!!! important
    ...
    **Never** update `.krupy-answers.yml` manually.

    This will trick Krupy, making it believe that those modified answers produced the
...
```shell
krupy update --defaults --data updated_question="my new answer"
```
...
As you can see here, `krupy` does several things:
...
Usually Krupy will replay the last project generation without problems. However,
...
-   When the old and new versions of the template depend on different incompatible
    versions of the same Jinja extension, but Krupy can only use one.
-   When the old version of the template was built for an older version of Krupy.

Generally, you should keep your templates as pure and simple as possible to avoid those
situations. But it can still happen.

To overcome this, use [the krupy recopy command][regenerating-a-project], which will
